...
This file formalizes the tree version of Simon's Factorization Forest Theorem,
deriving it from Simon's Split Theorem (`simon_word`).

Following Thomas Colcombet's presentation (*The Factorization Forest Theorem*, 2008/2012),
a factorization tree of a word `u ∈ A+` is an unranked ordered tree whose leaves are
labeled by the letters of `u` (so the yield/value of the tree is `u`).
A factorization tree is Ramsey for a semigroup morphism `eval : List A → S` if every
internal node either:
1. is a binary node (has two children, both Ramsey), or
2. is an idempotent node (all children have values evaluating under `eval` to the same
   idempotent `e ∈ S`, with `e * e = e` and at least 2 children).

## Main Definitions

* `FactorizationTree A` — an unranked ordered factorization tree over an alphabet `A`:
  - `leaf a`: a single leaf labeled by a letter `a : A`
  - `binary l r`: a binary node combining two subtrees
  - `idempotent children`: an n-ary node with children evaluating to the same idempotent
* `FactorizationTree.value` — the word in `List A` obtained by reading the leaves from
  left to right (the yield of the tree)
* `FactorizationTree.height` — the height (edge count) of the tree (leaves have height 0)
* `FactorizationTree.IsRamsey` — predicate verifying that the tree is Ramsey for `eval`

## Main Results

* `factorization_forest_theorem` — **Simon's Factorization Forest Theorem**:
  every non-empty word `u ∈ List A` evaluated into a finite semigroup `S` admits a
  Ramsey factorization tree with yield `u` and height at most `3 * nS S - 1`.

## References

* [T. Colcombet, *The Factorization Forest Theorem*][colcombet2008]
* [I. Simon, *Factorization Forests of Finite Height*][simon1990]
-/
...
mutual
  /-- A factorization tree is Ramsey for a semigroup morphism `eval` if:
  1. Leaves are always Ramsey.
  2. Binary nodes have Ramsey left and right children.
  3. Idempotent nodes have at least 2 children, all children are Ramsey, and the
     values of all children evaluate under `eval` to the same idempotent `e ∈ S`. -/
...
/-- Inner induction lemma for constructing idempotent factorization trees.
Given two boundary points `i < j` of rank `m` with all intermediate points having rank
at most `m`, constructs a non-empty list of trees of height `≤ 3 * m` whose yields concatenate
to `u[i..j]` and each evaluate under `eval` to the same idempotent
`(wordLabeling eval hmul u).σ i j`. If there is at least one intermediate cut point of
rank `m`, the resulting list has length at least 2. -/
  ...
  have H : ∀ (len : ℕ) (i j : Fin (u.length + 1))
    ...
    | h len ih_strong =>
      ...
      · let S_cuts :=
        ...
        have h_trees_ramsey :
          FactorizationTree.listIsRamsey eval (t_outer :: trees_inner) := by
...
/-- Outer induction lemma for constructing factorization trees level by level.
Proves that every subsegment between `i < j` whose internal points all have rank
strictly less than `m` admits a Ramsey factorization tree of yield `u[i..j]` and
height at most `3 * m`. -/
lemma split_to_tree_outer {n : ℕ}
  ...
  | zero =>
    ...
    have hj_eq : (j : ℕ) = (i : ℕ) + 1 := by
      ...
      have hix : (i : ℕ) < (x : ℕ) := by dsimp [x]; omega
      have hxj : (x : ℕ) < (j : ℕ) := by dsimp [x]; omega
  ...
  | succ m' ih_m' =>
    ...
    · have h_less' : ∀ x :
      ...
      refine ⟨t, ht_val, ht_ramsey, by omega⟩
    · have h_nonempty : S_cuts.Nonempty := Finset.nonempty_of_ne_empty h_empty
      ...
      have h_less_krj : ∀ x :
        ...
        intro x hkx hxj
        have _ : k_1 ≤ k_r := Finset.min'_le S_cuts k_r hkr_mem
        have _ := h_less x (by omega) hxj
        by_contra hc
        have h_eq : (s x : ℕ) = m' := by omega
        have h_mem : x ∈ S_cuts :=
      ...
      · have hk1j : (k_1 : ℕ) < (j : ℕ) := by rw [h_eq]; exact hkrj
        ...
        obtain ⟨t_k1j, ht_k1j_val, ht_k1j_ramsey, ht_k1j_height⟩ :=
          ih_m' (by omega) k_1 j hk1j h_less_k1j
        let t := FactorizationTree.binary t_ik1 t_k1j
        have ht_val : t.value = (u.drop i).take (j - i) := by
          dsimp [t, FactorizationTree.value]
          rw [ht_ik1_val, ht_k1j_val]
          exact list_drop_take_append u i.val k_1.val j.val hik1.le hk1j.le
        have ht_ramsey : t.IsRamsey eval :=
          FactorizationTree.binary_isRamsey eval ht_ik1_ramsey ht_k1j_ramsey
...
/-- **Simon's Factorization Forest Theorem** (Colcombet Theorem 3.4):
For all alphabets `A`, all finite semigroups `S`, and all semigroup morphisms
`eval : List A → S` (satisfying `eval (u ++ v) = eval u * eval v`), every non-empty
word `u ∈ List A` admits a Ramsey factorisation tree of value `u` and of height
at most `3 * nS S - 1`. -/
theorem factorization_forest_theorem {A S : Type*} [Semigroup S] [Fintype S]
  ...
  haveI : Nonempty (Fin (nS S)) := instNonemptyFin_nS
  ...
  have hij : (i : ℕ) < (j : ℕ) := by
    ...
    | nil => contradiction
    | cons a rest => simp
  have h_bound_all : ∀ x : Fin (u.length + 1), (s x : ℕ) ≤ m := by
    intro x
    have hs_lt := (s x).isLt
    omega
  have hsi : (s i : ℕ) = m := by
